...
var GJ_MINI_BAR_ENGINE = {
	...
	getAdcJson: function()
	{
		...
		if(IsIE())
		{
			if (typeof(window.TsCmcObj) == "object") {
				if (typeof(window.TsCmcObj.GetAdRule) != "undefined" && typeof(window.TsCmcObj.GetAdRule) != "unknown") {
					strCss = window.TsCmcObj.GetAdRule(window.location.href);
		...
		{
			chrome.extension.sendRequest({cmd: "getADCJson", url: location.href}, 
				function(response) {
					if (response ) {
	  ...
	  var vy = GJ_MINI_BAR_ENGINE.getTop($gjjq('#gj_mini').get(0))+ 20;
	   ...
	   if(IsIE())
	   {
		   if (typeof(window.TsCmcObj.ADCallBack) != "undefined" && typeof(window.TsCmcObj.ADCallBack) != "unknown") {
				 window.TsCmcObj.ADCallBack(window.location.href,GJ_MINI_BAR_ENGINE.nAdCount);
				}
	   }
	   else
	   {
            chrome.extension.sendRequest({cmd: "ADCallBack", url: window.location.href, count: GJ_MINI_BAR_ENGINE.nAdCount}, 
				function(response) {
			     });
	   }
    ...
    report: function(type,async,append) {
        ...
        g.rand = Math.random();
        g.refer = encodeURIComponent(location.href);
   ...
   /* callsetting: function() {
    ...
    InsertFrame: function() {
		...
		var strRefer = encodeURIComponent(location.href);
         var PannelUrl = "";
        if(IsIE())
        {
